    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[85vh]">
        ...
        <ScrollArea className="max-h-[65vh]">
          {isGroupPhase ? (
            <GroupView matches={phaseMatches} />
          ) : (
            <KnockoutView matches={phaseMatches} phaseName={currentPhase?.name ?? ''} />
          )}
        </ScrollArea>
      </DialogContent>
    </Dialog>
...
const GroupView = ({ matches }: { matches: Match[] }) => {
  ...
  if (sortedGroups.length === 0) {
    return <p className="text-center text-muted-foreground py-8">Nenhum jogo cadastrado nesta fase</p>;
  }
        ...
        {matches.map((m, i) => (
          <div key={m.id} className="relative">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-xs font-display font-bold text-primary shrink-0">
                {i + 1}
              </div>
              <div className="flex-1 rounded-lg border border-primary/20 bg-card/80 overflow-hidden">
                <div className="grid grid-cols-[1fr_auto_1fr]">
                  <div className={`p-3 flex items-center justify-end gap-2 ${
                    m.is_finished && m.score_a !== null && m.score_b !== null && m.score_a > m.score_b
                      ? 'bg-primary/10' : ''
                  }`}>
                    ...
                    {m.is_finished && (
                      <span className="font-display font-bold text-lg text-primary">{m.score_a}</span>
                    )}
                  </div>
                  ...
                  <div className={`p-3 flex items-center gap-2 ${
                    m.is_finished && m.score_a !== null && m.score_b !== null && m.score_b > m.score_a
                      ? 'bg-primary/10' : ''
                  }`}>
                    {m.is_finished && (
                      <span className="font-display font-bold text-lg text-primary">{m.score_b}</span>
                    )}
                    <span className="text-sm font-medium truncate">
                      <TeamName name={m.team_b} side="right" />
                    </span>
